    ...
    <script>
        ...
        function addToCart(name, price, image, event, slug) {{
            ...
            if (existing) {{ 
                ...
                cart.push({{name, price: parseFloat(price), image, slug: slug || '', qty: 1}}); 
            }}
            saveCart(cart);
        ...
        function buyNow(name, price, image, event, slug) {{
            if(event) event.stopPropagation();
            window.location.href = '/checkout.html?buy_now=true&product=' + encodeURIComponent(name) + '&price=' + price + '&slug=' + encodeURIComponent(slug || '');
        }}
...
def process_woocommerce_csv():
    ...
    for i, prod in enumerate(products_list):
        ...
        product_url = f"https://www.asmveo.com/product/{prod['slug']}.html"
        wa_text = f"Hi, I want to order {prod['name']} (Rs {prod['final_price']}).\nProduct: {product_url}\nIs it available?"
   ...
   # ==============================================================================
    ...
    <script>
        ...
        function updateCheckoutWhatsApp() {
            const urlParams = new URLSearchParams(window.location.search);
            const base = 'https://www.asmveo.com';
            let lines = ['Hi ASM VEO, I want to place an order:'];
            if (urlParams.get('buy_now') === 'true') {
                const name = urlParams.get('product') || '';
                const price = urlParams.get('price') || '';
                const slug = urlParams.get('slug') || '';
                lines.push('1x ' + name + ' (Rs ' + price + ')');
                if (slug) lines.push('Product: ' + base + '/product/' + encodeURIComponent(slug) + '.html');
            } else {
                let cart = getCart();
                cart.forEach(item => {
                    const qty = item.qty || 1;
                    lines.push(qty + 'x ' + item.name + ' (Rs ' + (Number(item.price) * qty) + ')');
                    if (item.slug) lines.push('Product: ' + base + '/product/' + encodeURIComponent(item.slug) + '.html');
                });
            }
        ...
        function renderCart() {
            ...
            const pSlug = urlParams.get('slug') || '';
            const siteBase = 'https://www.asmveo.com';
            ...
            if (isBuyNow && pName && pPrice) {
                subtotal = pPrice; 
                finalOrderString = "1x " + pName + " (Rs " + pPrice + ")" + (pSlug ? "\nProduct: " + siteBase + "/product/" + encodeURIComponent(pSlug) + ".html" : ""); 
        ...
        document.getElementById('checkoutForm').addEventListener('submit', function(e) {
            ...
            // Original working ASM tracking ID flow.
            let oId = 'ASM-' + Math.floor(100000 + Math.random() * 900000);
            const formData = new FormData(this);
            formData.append("Tracking_ID", oId);

            // Product links are already included in Product_Ordered; also send a dedicated field.
            const productDetails = document.getElementById('productField').value || '';
            formData.append("Product_Links", productDetails);

            // Original Google Apps Script / Google Sheets integration.
            let orderData = {
                orderId: oId,
            ...
            // Fire both integrations independently so a slow/failing Sheets request cannot block email.
            const sheetPromise = fetch(GOOGLE_SHEET_URL, {
                ...
                console.log("Order sent to Google Sheets database.");
                return true;
            }).catch(err => {
            ...
            // Keep a local backup exactly as before.
            try {
            ...
            // Original working Formspree email integration.
            const emailPromise = fetch('https://formspree.io/f/xjgnlgpw', {
                method: 'POST',
                body: formData,
            ...
            // Do not make one integration's failure automatically cancel the other.
            Promise.all([sheetPromise, emailPromise]).then(([sheetOk, emailOk]) => {
                if (!sheetOk && !emailOk) {
                    showToast('Network Error! Order was saved on this device. Please use WhatsApp to confirm.', 'fa-wifi', 'red');
                    btn.innerHTML = '<i class="fas fa-check-circle" aria-hidden="true"></i> Confirm Order';
                    btn.disabled = false;
                    return;
                }

                if (!sheetOk) console.warn('Email succeeded, but Google Sheets did not confirm the request.');
                if (!emailOk) console.warn('Google Sheets request completed, but email did not succeed.');

                if (typeof sendTrustpilotInvitation === 'function') sendTrustpilotInvitation();
