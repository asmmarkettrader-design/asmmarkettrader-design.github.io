...
def generate_pagination_html(current_page, total_pages, url_pattern):
    ...
    for n in pages:
        if n == "...":
            ...
            html += f'<span class="px-3 py-2 rounded-lg bg-[#E53935] text-white" aria-current="page">{n}</span>'
        else:
    ...
    for i, prod in enumerate(products_list):
        ...
        for idx, (kind, media_url) in enumerate(media_items):
            if kind == 'video':
                ...
                gallery_thumbs += f'''<button type="button" onclick="playProductVideo('{safe_video}')" class="relative flex-shrink-0 w-16 h-16 rounded-lg overflow-hidden border-2 border-gray-200 hover:border-[#E53935]" aria-label="Play product video" title="Product Video">
                ...
                gallery_thumbs += f'''<img src="{media_url}" alt="{safe_prod_name} view {idx+1}" onclick="changeMainImage(this)" class="w-16 h-16 object-cover rounded-lg cursor-pointer border-2 {border_class} hover:border-[#E53935] transition" loading="lazy" decoding="async" onerror="this.style.display='none'">'''
        ...
        <div class="container mx-auto px-4 py-10">
            <nav class="text-sm text-gray-600 dark:text-gray-400 mb-6 font-semibold bg-gray-100 dark:bg-gray-800 p-3 rounded-lg inline-block" aria-label="Breadcrumb">
                ...
                <a href="/category/{category_slug(prod['category'])}.html" class="hover:text-[#E53935] transition">{prod['category']}</a> &gt; 
...
# ================= CATEGORY PAGES =================
    ...
    for cat_name, prods in sections_dict.items():
        ...
        subnav_html = ''.join(
            ...
            f'<img src="{next((x.get("image") for x in prods if x.get("subcategory")==sub and x.get("image")), "/icon.png")}" alt="{sub}" class="w-12 h-12 md:w-14 md:h-14 rounded-full object-cover mb-1" loading="lazy">'
...
<div class="grid grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-2 md:gap-4">"""
   ...
   # ==============================================================================
    ...
    HOME_MAIN_CATEGORY_RULES = [
        ("Fashion", ["fashion","apparel","cloth","clothing","dress","wear","garment","kurta","shalwar","hijab","abaya"]),
        ("Electronics", ["electronic","laptop","computer","tablet","earbud","headphone","smartwatch","gadget","camera","speaker","projector"]),
        ("Mobile Accessories", ["mobile accessory","mobile accessories","phone accessory","iphone cover","phone cover","charger","charging cable","power bank","mobile case","screen protector","handsfree"]),
        ("Beauty & Personal Care", ["beauty","cosmetic","makeup","skin","skincare","hair care","perfume","fragrance","personal care","serum","cream","shampoo"]),
        ("Home & Kitchen", ["home","kitchen","living","furniture","decor","decoration","bedsheet","curtain","cookware","appliance","storage"]),
        ("Health & Wellness", ["health","wellness","medical","fitness","supplement","vitamin","exercise","gym","healthcare"]),
        ("Kids & Babies", ["kid","kids","baby","babies","toy","toys","children","school","baby care","feeding"])
    ]
    fixed_home=[]
    ...
    for h_page in range(1, total_home_pages + 1):
        ...
        if h_page == 1:
            ...
            # ===== FLAT SALE: 3 TOP PRODUCTS PER MAIN CATEGORY, AUTO-SLIDING =====
            flat_sale_groups=[]
            for fs_cat, fs_prods in all_categories_list[:7]:
                fs_picks=[p for p in fs_prods if p.get("image") and p.get("slug")][:3]
                if fs_picks: flat_sale_groups.append((fs_cat,fs_picks))
            home_html += '<section class="container mx-auto px-4 py-5 mt-5"><div class="flex items-center justify-between mb-4"><div><h2 class="text-2xl font-black text-gray-900 dark:text-white">🔥 Flat Sale</h2><p class="text-xs text-gray-500 dark:text-gray-400">Top picks from our main categories</p></div><span class="text-xs font-bold text-[#E53935]">Auto Slide</span></div><div id="flatSaleViewport" class="overflow-hidden rounded-2xl"><div id="flatSaleTrack" class="flex">'
            for fs_i,(fs_cat,fs_picks) in enumerate(flat_sale_groups):
                home_html += f'<div class="min-w-full grid grid-cols-3 gap-3 bg-white dark:bg-gray-800 p-2" data-flat-sale="{fs_i}">'
                for fs_p in fs_picks: home_html += generate_product_card(fs_p,lazy=(fs_i!=0))
                home_html += '</div>'
            home_html += '</div></div></section><script>(function(){const t=document.getElementById("flatSaleTrack"),g=t?t.querySelectorAll("[data-flat-sale]"):[];let i=0;if(g.length>1)setInterval(()=>{i=(i+1)%g.length;t.style.transform="translateX(-"+(i*100)+"%)";},4500);})();</script>'
                        ...
                        html += `<div class="product-card reveal active bg-white dark:bg-gray-800 rounded-lg shadow-sm hover:shadow-md border border-gray-200 dark:border-gray-700 overflow-hidden flex flex-col relative cursor-pointer" onclick="window.location.href='/product/${p.slug}.html'">
                            ...
                            <div class="p-2 flex flex-col flex-grow">
                                ...
                                <div class="mt-auto">
                                    ...
                                    <button aria-label="Add Searched Item to Cart" onclick="addToCart('${jsSafeName}', ${p.final_price}, '${p.image}', event, '${p.slug}')" class="w-full bg-gray-50 text-[#E53935] py-1.5 rounded-md text-[10px] font-bold border border-gray-200 hover:bg-[#E53935] hover:text-white transition flex justify-center items-center"><i class="fas fa-cart-plus" aria-hidden="true"></i></button>
                                </div>
                            </div>
                        </div>`;
                    });
...
function getDeliveryInfo(city){const z=deliveryZones[city]||[DELIVERY_FEE,3,5];return{charge:z[0],min:z[1],max:z[2]};}
function getCheckoutItems(){const u=new URLSearchParams(location.search);if(u.get('buy_now')==='true'){const name=u.get('product')||'',price=Number(u.get('price'))||0,slug=u.get('slug')||'';return name&&price>0?[{name,price,qty:1,image:'',slug}]:[];}return(getCart()||[]).map(i=>({name:String(i.name||''),price:Number(i.price)||0,qty:Math.max(1,Number(i.qty)||1),image:i.image||'',slug:i.slug||''})).filter(i=>i.name&&i.price>0);}
function updateDeliveryEstimate(){const city=document.getElementById('citySelect')?.value||'',z=getDeliveryInfo(city),n=document.getElementById('deliveryEstimate');if(n)n.textContent=city?('Estimated delivery: '+z.min+'-'+z.max+' working days • Rs '+z.charge):'Select your city for delivery estimate';renderCart();}
function togglePaymentDetails(){const r=document.querySelector('input[name="Payment_Method"]:checked');if(!r)return;const adv=r.value==='Advance Payment',d=document.getElementById('advancePaymentDetails'),c=document.getElementById('labelCOD'),a=document.getElementById('labelAdv');d.classList.toggle('hidden',!adv);a.classList.toggle('border-[#E53935]',adv);a.classList.toggle('bg-red-50',adv);c.classList.toggle('border-[#E53935]',!adv);c.classList.toggle('bg-red-50',!adv);renderCart();}
function applyCoupon(){const code=(document.getElementById('couponCode')?.value||'').trim().toUpperCase(),sub=getCheckoutItems().reduce((s,i)=>s+i.price*i.qty,0);if(code==='ASM10'&&sub>=3000){couponApplied=true;showToast('Coupon applied! 10% discount added.','fa-check-circle','pk');}else{couponApplied=false;showToast(code==='ASM10'?'Minimum Rs 3000 shopping required for this coupon.':'Invalid coupon code.','fa-exclamation-circle','red');}renderCart();}
function updateCheckoutWhatsApp(){const items=getCheckoutItems(),city=document.getElementById('citySelect')?.value||'',z=getDeliveryInfo(city),sub=items.reduce((s,i)=>s+i.price*i.qty,0),disc=couponApplied?Math.floor(sub*.10):0,total=sub-disc+z.charge,pay=document.querySelector('input[name="Payment_Method"]:checked')?.value||'Cash on Delivery';let lines=['Hi ASM VEO, I want to place an order:'];items.forEach(i=>{lines.push(i.qty+'x '+i.name+' (Rs '+(i.price*i.qty)+')');if(i.slug)lines.push('Product: https://www.asmveo.com/product/'+encodeURIComponent(i.slug)+'.html');});lines.push('Subtotal: Rs '+sub,'Delivery Charges: Rs '+z.charge,'Discount: Rs '+disc,'Grand Total: Rs '+total,'Payment Method: '+pay);const el=document.getElementById('checkoutWhatsApp');if(el)el.href='https://wa.me/923425478683?text='+encodeURIComponent(lines.join('\n'));}
function renderCart(){const items=getCheckoutItems(),container=document.getElementById('cartItemsContainer');if(!container)return;let sub=0;container.innerHTML='';const buy=new URLSearchParams(location.search).get('buy_now')==='true';if(!items.length){container.innerHTML='<div class="text-center py-8"><p class="text-gray-500 font-semibold">Your cart is empty.</p><a href="/index.html" class="inline-block mt-4 bg-[#E53935] text-white px-6 py-2 rounded-xl font-bold">Browse Products</a></div>';document.getElementById('submitBtn').disabled=true;}else{document.getElementById('submitBtn').disabled=false;items.forEach((i,idx)=>{const line=i.price*i.qty;sub+=line;const safe=String(i.name).replace(/[&<>"']/g,ch=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[ch]));const controls=buy?'':'<div class="flex items-center gap-2 mt-1"><button type="button" onclick="updateQty('+idx+',-1)" class="w-7 h-7 bg-gray-200 rounded">−</button><span>'+i.qty+'</span><button type="button" onclick="updateQty('+idx+',1)" class="w-7 h-7 bg-gray-200 rounded">+</button><button type="button" onclick="removeFromCart('+idx+')" class="ml-2 text-red-500"><i class="fas fa-trash"></i></button></div>';container.innerHTML+='<div class="flex items-center gap-3 bg-gray-50 dark:bg-gray-700 p-3 rounded-xl border border-gray-200 dark:border-gray-600">'+(i.image?'<img src="'+i.image+'" class="w-16 h-16 object-contain rounded-lg bg-white border p-1" loading="lazy" alt="Cart Item">':'')+'<div class="flex-1 min-w-0"><h3 class="font-bold text-sm text-gray-900 dark:text-white line-clamp-2">'+safe+'</h3><p class="text-xs text-gray-500">Rs '+i.price+' × '+i.qty+'</p><p class="text-[#E53935] font-black">Rs '+line+'</p>'+controls+'</div></div>';});}const city=document.getElementById('citySelect')?.value||'',z=getDeliveryInfo(city),disc=couponApplied?Math.floor(sub*.10):0,total=sub-disc+z.charge;document.getElementById('subtotalDisplay').innerText='Rs '+sub;document.getElementById('deliveryDisplay').innerText='Rs '+z.charge;document.getElementById('grandTotalDisplay').innerText='Rs '+total;const dr=document.getElementById('discountRow');if(disc>0){dr.classList.remove('hidden');document.getElementById('discountDisplay').innerText='- Rs '+disc;}else dr.classList.add('hidden');const pay=document.querySelector('input[name="Payment_Method"]:checked')?.value||'Cash on Delivery';const lines=items.map(i=>i.qty+'x '+i.name+' (Unit Rs '+i.price+', Line Rs '+(i.price*i.qty)+')'+(i.slug?'\nProduct: https://www.asmveo.com/product/'+encodeURIComponent(i.slug)+'.html':''));document.getElementById('productField').value=lines.join('\n')+'\nSubtotal: Rs '+sub+'\nDelivery Charges: Rs '+z.charge+'\nDiscount: Rs '+disc+'\nGrand Total: Rs '+total+'\nPayment Method: '+pay;document.getElementById('totalField').value='Rs '+total;updateCheckoutWhatsApp();}

document.getElementById('checkoutForm').addEventListener('submit',async function(e){e.preventDefault();e.stopPropagation();const form=this,btn=document.getElementById('submitBtn');if(!form.checkValidity()){form.reportValidity();return;}const items=getCheckoutItems();if(!items.length){showToast('Your cart is empty.','fa-cart-shopping','red');return;}renderCart();const orderId='ASM-'+Math.floor(100000+Math.random()*900000),formData=new FormData(form);formData.append('Tracking_ID',orderId);formData.append('Product_Links',document.getElementById('productField').value);const orderData={orderId:orderId,name:document.getElementById('fullName').value.trim(),phone:document.getElementById('phoneNum').value.trim(),city:document.getElementById('citySelect').value,address:document.getElementById('addressInput').value.trim(),email:document.getElementById('emailAddr').value.trim(),paymentMethod:document.querySelector('input[name="Payment_Method"]:checked')?.value||'',products:document.getElementById('productField').value,productLinks:document.getElementById('productField').value,total:document.getElementById('totalField').value};btn.disabled=true;btn.innerHTML='<i class="fas fa-spinner fa-spin"></i> Processing Order...';try{let saved=JSON.parse(localStorage.getItem('asm_orders')||'[]');saved.unshift({orderId,total:orderData.total,products:orderData.products,status:'Submitted',createdAt:new Date().toISOString()});localStorage.setItem('asm_orders',JSON.stringify(saved.slice(0,20)));}catch(_){ }const sheetPromise=fetch('https://script.google.com/macros/s/AKfycbxDcasUtmgv79TYIhNY3jaT6HJ5UHwEAhmHtlki0-6Uy3v6NfKzblwMJ6Ro-bR9l7Es/exec',{method:'POST',mode:'no-cors',headers:{'Content-Type':'application/json'},body:JSON.stringify(orderData)}).then(()=>true).catch(err=>{console.error('Google Sheets error',err);return false;});const emailPromise=fetch('https://formspree.io/f/xjgnlgpw',{method:'POST',body:formData,headers:{'Accept':'application/json'}}).then(r=>{if(!r.ok)throw new Error('Email HTTP '+r.status);return true;}).catch(err=>{console.error('Email error',err);return false;});const[sheetOk,emailOk]=await Promise.all([sheetPromise,emailPromise]);if(!sheetOk&&!emailOk){showToast('Network issue: order saved on this device. Please confirm by WhatsApp.','fa-wifi','red');btn.disabled=false;btn.innerHTML='<i class="fas fa-check-circle"></i> Confirm Order';return;}try{const ce=document.getElementById('emailAddr').value.trim();if(ce)localStorage.setItem('asm_customer_email',ce);}catch(_){ }if(new URLSearchParams(location.search).get('buy_now')!=='true')localStorage.removeItem('asm_cart');if(typeof updateCartBadge==='function')updateCartBadge();window.location.href='/order-success.html?order='+encodeURIComponent(orderId);});
window.addEventListener('load',function(){try{const u=JSON.parse(localStorage.getItem('asm_account')||'null');if(u){if(u.name)fullName.value=u.name;if(u.email)emailAddr.value=u.email;if(u.phone)phoneNum.value=u.phone;if(u.address)addressInput.value=u.address;}}catch(_){ }togglePaymentDetails();renderCart();updateDeliveryEstimate();});
</script>
"""
