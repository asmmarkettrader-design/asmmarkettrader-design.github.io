...
<link rel="apple-touch-icon" href="/icon.png">
    ...
    <script>
        ...
        function loadSearchData() {{ if(searchLoaded) return; searchLoaded=true; let script=document.createElement('script'); script.src='/search-data.js'; script.defer=true; document.head.appendChild(script); }}
        function loadSearchSuggestions() {{ if(searchSuggestLoaded) return; searchSuggestLoaded=true; let script=document.createElement('script'); script.src='/search-suggest-data.js'; script.defer=true; document.head.appendChild(script); }}
        function showSearchSuggestions(value) {{
            loadSearchSuggestions(); const box=document.getElementById('searchSuggestions'); if(!box)return; const q=String(value||'').trim().toLowerCase();
            if(!q || typeof searchSuggestions==='undefined'){{box.classList.add('hidden');return;}}
            const words=q.split(/\\s+/).filter(Boolean); const matches=searchSuggestions.filter(x=>words.every(w=>x.text.includes(w))).slice(0,7);
            if(!matches.length){{box.classList.add('hidden');return;}}
            box.innerHTML=matches.map(x=>`<a href="${{x.url}}" class="block px-4 py-3 hover:bg-gray-50 dark:hover:bg-gray-700 border-b border-gray-100 dark:border-gray-700"><b class="text-gray-900 dark:text-white">${{x.name}}</b><span class="block text-xs text-gray-500">${{x.category}}</span></a>`).join(''); box.classList.remove('hidden');
        }}
