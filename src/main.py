...
<head>
    ...
    <meta name="keywords" content="buy {title} in Pakistan, {title} price in Pakistan, online shopping Pakistan, cash on delivery, ASM VEO, Karachi, Lahore, Islamabad">
    ...
    <meta property="og:description" content="Shop {title} online in Pakistan. Cash on Delivery available.">
    ...
    <script>
        tailwind.config = {{
            darkMode: 'class',
            theme: {{
                extend: {{ colors: {{ brand: {{ 50: '#ecfdf5', 500: '#10b981', 600: '#059669', 700: '#047857', 800: '#065f46', 900: '#064e3b' }} }} }}
            }}
        }}
    </script>
    ...
    <script>
        // GLOBAL API FOR SEARCH AND FILTERS
        window.GLOBAL_SEARCH_INDEX = [];
        let isFetchingSearch = false;

        async function fetchSearchData() {{
            if(window.GLOBAL_SEARCH_INDEX.length > 0 || isFetchingSearch) return;
            isFetchingSearch = true;
            try {{
                let res = await fetch('/search.json');
                window.GLOBAL_SEARCH_INDEX = await res.json();
                initLiveSales(); // Start Fake Sales Popup when data loads
            }} catch(e) {{ console.error("Search data failed to load."); }}
        }}
        ...
        function showToast(msg, icon='fa-check-circle', color='emerald') {{
            ...
            toast.className = `fixed bottom-24 md:bottom-4 right-4 ${{colors[color]}} text-white px-6 py-3 rounded-xl shadow-2xl z-[9999] transform transition-all duration-300 flex items-center gap-3 font-bold slide-in`;
        ...
        // LIVE SALES POPUP (Social Proof)
        function initLiveSales() {{
            setInterval(() => {{
                if(!window.GLOBAL_SEARCH_INDEX || window.GLOBAL_SEARCH_INDEX.length === 0) return;
                if(Math.random() > 0.6) return; // Only show randomly
                
                let prod = window.GLOBAL_SEARCH_INDEX[Math.floor(Math.random() * window.GLOBAL_SEARCH_INDEX.length)];
                let cities = ["Karachi", "Lahore", "Islamabad", "Rawalpindi", "Multan", "Faisalabad", "Peshawar"];
                let names = ["Ali", "Ayesha", "Usman", "Fatima", "Bilal", "Zainab", "Hassan"];
                let city = cities[Math.floor(Math.random() * cities.length)];
                let name = names[Math.floor(Math.random() * names.length)];
                
                let shortName = prod.name.length > 25 ? prod.name.substring(0, 25) + '...' : prod.name;
                let msg = name + ' from ' + city + ' just bought <br><span class="text-xs font-black">' + shortName + '</span>';
                
                showToast(msg, 'fa-shopping-bag', 'emerald');
        ...
        window.onload = function() {{
            updateCartBadge();
            updateWishlistBadge();
            fetchSearchData(); // Silently preload JSON Data for speed
...
<body class="text-gray-900 dark:text-gray-100">
    ...
    <a href="https://wa.me/923425478683?text=Hi,%20I%20want%20to%20know%20about%20your%20products" target="_blank" 
       class="fixed bottom-24 md:bottom-10 right-4 bg-green-500 text-white w-14 h-14 rounded-full shadow-2xl flex items-center justify-center hover:bg-green-600 transition-all z-[90] hover:scale-110" 
    ...
    <button id="backToTop" onclick="scrollTop()" class="hidden fixed bottom-40 md:bottom-28 right-4 md:left-4 bg-emerald-600 text-white w-12 h-12 rounded-full shadow-2xl items-center justify-center hover:bg-emerald-700 transition z-[90]" aria-label="Back to top">
        ...
        <div class="p-4 flex flex-col flex-grow">
            ...
            <div class="mt-auto">
                ...
                <div class="flex gap-2 w-full">
                    ...
                    <!-- BUG FIX: Removed extra quote after final_price -->
...
def process_woocommerce_csv():
    ...
    with open(file_path, mode='r', encoding='utf-8') as f:
        ...
        for row in reader:
            ...
            # BUG FIX: STRICT IMAGE VALIDATION (No 'http' = No Product)
    ...
    # BUG FIX: JSON EXTRACTED TO FILE (Fixes GitHub Size Limit Error)
        ...
        <div class="container mx-auto px-4 py-10">
            ...
            <div class="bg-white dark:bg-gray-800 rounded-3xl shadow-xl border border-gray-200 dark:border-gray-700 overflow-hidden flex flex-col md:flex-row mb-12">
                ...
                <div class="md:w-1/2 p-8 md:p-12 flex flex-col justify-center">
                    ...
                    <a href="https://wa.me/923425478683?text={whatsapp_msg.replace(' ', '%20')}" class="w-full md:w-5/6 mt-3 bg-green-500 text-white py-4 rounded-xl font-black text-lg hover:bg-green-600 transition-all shadow-lg flex justify-center items-center gap-2 transform hover:-translate-y-1">
    ...
    for cat_name, prods in sections_dict.items():
        ...
        # BUG FIX: Use Global JSON instead of embedding in every page
        cat_script = f"""
        <script>
            let currentCategory = '{cat_name}';
            
            async function applyFilters() {{
                await fetchSearchData();
                let allProducts = window.GLOBAL_SEARCH_INDEX.filter(p => p.category === currentCategory);
            ...
            function generateCard(p) {{
                ...
                return `<div class="product-card bg-white dark:bg-gray-800 rounded-2xl shadow-sm hover:shadow-xl border border-gray-200 dark:border-gray-700 overflow-hidden flex flex-col relative cursor-pointer" onclick="window.location.href='/product/${{p.slug}}.html'">
                    ...
                    <button onclick="quickView('${{safeName}}', ${{p.final_price}}, '${{p.image}}', '', '${{p.slug}}')" class="absolute top-3 right-14 w-9 h-9 bg-white rounded-full shadow-md flex items-center justify-center hover:bg-emerald-50 transition z-10"><i class="fas fa-eye text-emerald-600"></i></button>
    ...
    <script>
        async function performSearch(query) {
            ...
            await fetchSearchData();
            let searchIndex = window.GLOBAL_SEARCH_INDEX;
                ...
                <form id="checkoutForm" class="bg-white dark:bg-gray-800 p-6 md:p-8 rounded-b-3xl shadow-xl border border-gray-200 dark:border-gray-700 border-t-0 space-y-5">
                    ...
                    <div>
                        ...
                        <div class="flex gap-2">
                            ...
                            <button type="button" onclick="applyCoupon()" class="bg-gray-900 dark:bg-emerald-600 text-white px-5 rounded-xl font-bold hover:bg-gray-700 transition">Apply</button>
                        </div>
                    </div>
    ...
    print("🎉 Premium E-Commerce Engine Generated Successfully!")
    print(f"📦 Products: {len(products_list)} | 📂 Categories: {len(categories_list)} | 🏙️ Cities: {len(cities)}")
    print("✨ Features: Live Fake Sales Popups, Stock Scarcity Bar, Extracted JSON API, Direct WhatsApp Orders & More!")
