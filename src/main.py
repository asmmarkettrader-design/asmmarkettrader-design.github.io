...
# HTML Minifier for Fast Loading (From Script 1)
...
<head>
    ...
    <style>
        ...
        body {{ font-family: 'Plus Jakarta Sans', sans-serif; background-color: #f8fafc; transition: background-color 0.3s; padding-bottom: 70px; }}
    ...
    <script>
        ...
        window.onload = function() {{
            ...
            // External Search Data Load Integration (From Script 1)
            let sInput = document.getElementById('searchInput');
            if (sInput) {{ sInput.addEventListener('focus', loadSearchData); }}
        }};
        ...
        <div class="bg-gray-900 text-white text-xs md:text-sm py-2">
            <div class="container mx-auto px-4 flex justify-between items-center">
                <div class="flex space-x-4 items-center">
                    ...
                    <a href="/about.html" class="hover:text-emerald-400 transition font-semibold hidden md:inline"><i class="fas fa-info-circle mr-1"></i> About</a>
                    <a href="/contact.html" class="hover:text-emerald-400 transition font-semibold hidden md:inline"><i class="fas fa-envelope mr-1"></i> Contact</a>
        ...
        <div class="container mx-auto px-4 grid grid-cols-1 md:grid-cols-4 gap-10 mb-10">
            ...
            <div>
                ...
                <ul class="space-y-3 text-gray-400 text-sm font-semibold">
                    ...
                    <li><a href="/track-order.html" class="hover:text-emerald-400 transition"><i class="fas fa-angle-right mr-2 text-emerald-600"></i> Track Order</a></li>
                    <li><a href="/shipping-policy.html" class="hover:text-emerald-400 transition"><i class="fas fa-angle-right mr-2 text-emerald-600"></i> Shipping Policy</a></li>
                    <li><a href="/return-policy.html" class="hover:text-emerald-400 transition"><i class="fas fa-angle-right mr-2 text-emerald-600"></i> Return Policy</a></li>
                </ul>
            </div>
            <div>
...
def generate_static_pages(categories_list):
    ...
    pages = {
        ...
        <div class="container mx-auto px-4 py-16 max-w-4xl">
            <h1 class="text-4xl font-extrabold mb-8 text-gray-900 dark:text-white">Shipping Policy</h1>
            <div class="bg-white dark:bg-gray-800 rounded-3xl shadow-xl p-8 md:p-12 border border-gray-100 dark:border-gray-700 space-y-6 text-gray-600 dark:text-gray-300 text-sm leading-relaxed">
                ...
                <p>All orders are dispatched via trusted courier services. You will receive a tracking ID via SMS/WhatsApp once dispatched.</p>
            </div>
        </div>
        """),
        "return-policy.html": ("Return Policy", """
        <div class="container mx-auto px-4 py-16 max-w-4xl">
            <h1 class="text-4xl font-extrabold mb-8 text-gray-900 dark:text-white">Return Policy</h1>
            <div class="bg-white dark:bg-gray-800 rounded-3xl shadow-xl p-8 md:p-12 border border-gray-100 dark:border-gray-700 space-y-6 text-gray-600 dark:text-gray-300 text-sm leading-relaxed">
                ...
                <ul class="list-disc pl-6 space-y-2">
                    ...
                    <li>Refunds are processed to your Bank/JazzCash/Easypaisa within 5-7 days after we receive the returned item.</li>
                </ul>
            </div>
        </div>
        """),
        ...
        <div class="container mx-auto px-4 py-16 max-w-4xl text-center">
            ...
            <a href='https://wa.me/923425478683' class='bg-green-500 text-white px-8 py-4 rounded-xl font-bold hover:bg-green-600 transition shadow-lg'><i class="fab fa-whatsapp"></i> Track via WhatsApp</a>
        </div>
        """),
...
def process_woocommerce_csv():
    ...
    sitemap_urls = [
        ...
        "https://www.asmveo.com/track-order.html"
    ]
    ...
    for prod in products_list:
        ...
        <div id="stickyAddToCart" class="hidden fixed bottom-16 left-0 right-0 bg-white dark:bg-gray-900 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.1)] p-3 z-40 flex items-center justify-between md:hidden border-t border-gray-200 dark:border-gray-800">
            <span class="text-lg font-black text-emerald-800 dark:text-emerald-400">Rs {prod['final_price']}</span>
            <button onclick="addToCart('{escaped_name}', {prod['final_price']}, '{prod['image']}', event)" class="bg-emerald-600 text-white px-6 py-2.5 rounded-lg font-bold text-sm shadow-md">Add to Cart</button>
        </div>
        """
        ...
        <script>
            ...
            window.addEventListener('scroll', () => {{
                if (mainActions) {{
                    ...
                    if (rect.bottom < 0) stickyBar.classList.remove('hidden');
                    else stickyBar.classList.add('hidden');
                }}
            }});
        </script>
        """
        prod_html += prod_script + get_html_footer()
    ...
    for cat_name, prods in sections_dict.items():
            ...
            <div class="flex flex-col lg:flex-row gap-6">
                ...
                <div class="flex-1">
                    <div id="productGrid" class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-3 gap-4 md:gap-6">
                        <!-- Products injected by JS for Load More & Filtering -->
                    </div>
                    ...
                    <div class="text-center mt-8">
                        <button id="loadMoreBtn" onclick="loadMore()" class="bg-gray-100 dark:bg-gray-800 text-emerald-700 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-700 px-8 py-3 rounded-xl font-bold hover:bg-emerald-50 dark:hover:bg-emerald-900 transition" style="display: none;">Load More Products</button>
                    </div>
                </div>
            </div>
        </div>
        ...
        # Merge Load More logic with Filter logic dynamically using JS
        cat_products_minimal = [{"name": p['name'], "slug": p['slug'], "category": p['category'], "final_price": p['final_price'], "fake_price": p['fake_price'], "image": p['image']} for p in prods]
        
        cat_script = f"""
        <script>
            let allProducts = {json.dumps(cat_products_minimal)};
            let currentFiltered = [...allProducts];
            let currentRendered = 12;
            ...
            function renderGrid() {{
                ...
                let toRender = currentFiltered.slice(0, currentRendered);
                grid.innerHTML = toRender.map(p => generateCard(p)).join('');
                
                if (currentRendered >= currentFiltered.length) {{
                    btn.style.display = 'none';
                }} else {{
            ...
            function applyFilters() {{
                ...
                currentFiltered = allProducts.filter(p => p.final_price >= minP && p.final_price <= maxP);
                
                if (sortBy === 'price-low') currentFiltered.sort((a,b) => a.final_price - b.final_price);
                else if (sortBy === 'price-high') currentFiltered.sort((a,b) => b.final_price - a.final_price);
                else if (sortBy === 'name') currentFiltered.sort((a,b) => a.name.localeCompare(b.name));
            ...
            function resetFilters() {{
                ...
                document.getElementById('minPrice').value = '{int(min_price)}';
                document.getElementById('maxPrice').value = '{int(max_price)}';
                applyFilters();
            }}
        ...
        """
        cat_html += cat_script + get_html_footer()
    ...
    <script>
        function performSearch(query) {
            // Load external search index if not available
            ...
            let existingResults = document.querySelector('#searchResultsSection > div:not(:first-child)');
    ...
    print("✨ Features: HTML Minification, External Search, Hero Slider, Local SEO, Mobile Nav, Dark Mode, Filters & Load More!")
